...
var SelectServer = new Class({
	initialize : function(select_server_func){
		...
		for (var areaid in server_data){
			...
			for(var i=0; i < server_list.length; i++){
				if (server_list[i][0] == serverid){
					info = {"serverid": serverid, "server_name" : server_list[i][1]};
					break;
				}
			}
	...
	show_server_list : function(area_id){
		...
		var select_server = function(){
			...
			if (that.select_server_func){
				...
				var area_info = that.get_server_info(el.getAttribute("data_serverid"));
				var arg = {
					"act" : "show_login",
					"area_id" : el.getAttribute("data_areaid"),
					"area_name" : el.getAttribute("data_area_name"),
				...
				Cookie.write("area_id", area_id, {"duration":30})
				var url = "/cgi-bin/show_login.py?" + Object.toQueryString(arg);
