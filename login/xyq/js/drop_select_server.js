...
var DropSelectServer = new Class({
	...
	gen_select_box: function(box_obj, box_data, selected_id){
		...
		for(var i = 0; i < box_data.length; i++){
			var item = box_data[i];
			var op = new Element("option", {"value":item["value"], "text": item["text"]});
	...
	gen_area_box: function(){
		...
		var box_data = [];
		box_data.push({'value': '', 'text': '--ÇëÑ¡Ôñ--'});
		for (var i=0; i < area_list.length; i++){
			box_data.push({"value":area_list[i]["id"], "text":area_list[i]["name"]});
		}
	...
	gen_server_box: function(){
		...
		for (var i=0; i < server_list.length; i++){
			box_data.push({"value":server_list[i]["id"], "text":server_list[i]["name"]});
		}
		this.gen_select_box(this.server_el, box_data, "");
	}
});
