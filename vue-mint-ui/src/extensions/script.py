...
class ScriptExtension(BaseExtension):
  ...
  def parse(self, parser):
      ...
      return nodes.CallBlock(self.call_method('_add_inline_script', []), [], [], body).set_lineno(lineno)

  
  # 添加资源
  def _add_inline_script(self, caller):
    # 获取 {% script %}...{% endscript %} 语句中的内容
    # 这里 caller() 对应上面调用 CallBlock 时传入的 body
    content = caller()
    self.environment.require.add_inline_resource(content, 'js')
    return ''