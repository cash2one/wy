...
env = Environment(
    auto_reload=False,
    loader=FileSystemLoader('./pat'), # 也可以是绝对路径
...
env.require.ready() # 准备渲染，清空 env 的临时资源

template = env.get_template('index.html')
result = render(template, { "title": u"测试页面", "content": u"测试内容" })

obj = env.require.build()
result = result.replace('</head>', obj["style"] + '\n</head>')
result = result.replace('</body>', obj["script"] + '\n</body>')

env.require.reset() # 渲染完毕，清空 env 的临时资源
